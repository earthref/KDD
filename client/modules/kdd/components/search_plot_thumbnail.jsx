...
import SearchPlot from '/client/modules/kdd/containers/search_plot';
        ...
        <div style={thumbnailContainerStyle}>
          <a onClick={this.showPlots.bind(this)} style={{ cursor:'pointer', display:'flex', maxWidth: 100, maxHeight: 100, margin:'auto'}}>
            ...
            <div className="ui top right attached small basic label" style={{ padding:'.5rem', margin:'-1px', zIndex: 1000 }}>
              <span style={{ color: portals['GERM'].color }}>
                { count }
              </span>
            </div>
          </a>
            ...
            <div className="header actions" style={{ textAlign: 'left', padding: '.5rem' }}>
              <div className="ui small basic compact buttons">
                <div className="ui active button" style={{cursor: 'default'}}>
                  <span style={{ fontWeight: 'bold', color: portals['GERM'].color }}>
                    Data Model Level:
                  </span>
                </div>
                { this.levels.map((level, idx) => levels[level].count && 
                  ...
                  >
                    <span style={{fontWeight: 'bold', color: (level === activeLevel ? portals['GERM'].color : 'inherit') }}>
                      { level }
                    </span>
                    <div 
                      className={"ui horizontal label" + (level === activeLevel ? "" : " basic")}
                      style={{ margin: '-1rem -.75rem -1rem 0.75rem', padding: '.2rem .5rem', minWidth: 0, color: (level === activeLevel ? portals['GERM'].color : 'inherit') }}
                    >
                      { levels[level].count }
                    </div>
                  </div> || undefined
            ...
            <div className="actions" style={{ textAlign: 'left', padding: '.5rem' }}>
              <div className="ui small basic compact buttons">
                ...
                { _.keys(levels[activeLevel]).map((type, idx) => levels[activeLevel][type].length &&
                  ...
                  >
                    <span style={{fontWeight: 'bold', color: (type === activeType ? portals['GERM'].color : 'inherit')}}>
                      { type }
                    </span>
                    <div 
                      className={"ui horizontal label" + (type === activeType ? "" : " basic")}
                      style={{ margin: '-1rem -.75rem -1rem 0.75rem', padding: '.2rem .5rem', minWidth: 0, color: (type === activeType ? portals['GERM'].color : 'inherit') }}
                    >
