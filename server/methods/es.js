...
import ValidateContribution from '/lib/modules/kdd/validate_contribution.js';
import {versions} from '/lib/configs/kdd/data_models';
      ...
      try {
        ...
        if (isLatest === 'true') {
          history.slice(1).forEach(v => {
            try {
              s3DeleteKeys({
                bucket: 'kdd-contributions',
                keys: [`${v.id}/kdd_contribution_${v.id}.txt`]
              }).then(() => {
                console.log("esUploadActivatedContributionToS3", `Deleted ${v.id}/kdd_contribution_${v.id}.txt`);
                s3DeleteKeys({
                  bucket: 'kdd-contributions',
                  keys: [`${v.id}/`]
                }).then(() => {
          ...
          try {
            s3UploadObject({
                bucket: 'kdd-contributions',
                key: `${id}/kdd_contribution_${id}.txt`,
                body: contributionText
            }).then(() => {
              console.log("esUploadActivatedContributionToS3", `Uploaded ${id}/kdd_contribution_${id}.txt to kdd-contributions`);
            });
          } catch (e) {
        ...
        try {
          s3UploadObject({
            bucket: 'kdd-activated-contributions',
            key: `${id}/kdd_contribution_${id}.txt`,
            body: contributionText
          }).then(() => {
            console.log("esUploadActivatedContributionToS3", `Uploaded ${id}/kdd_contribution_${id}.txt to kdd-activated-contributions`);
          });
        } catch (e) {
